...
`is_constant` in implementation.py depends only on the `Executor` protocol,
not on Aer directly, so a future hardware or noise-aware backend is a
drop-in swap rather than a rewrite of the algorithm (same pattern as
